...
class Visitor(models.Model):
    """Library visitors/students for borrowing and e-library sessions"""

    full_name = models.CharField(max_length=200)
    department = models.CharField(max_length=100)
    level = models.CharField(max_length=50)  # Academic level (100, 200, etc.)
...
class ELibrarySession(models.Model):
    """E-Library session records for student visits"""

    visitor = models.ForeignKey(Visitor, on_delete=models.CASCADE, related_name="elibrary_sessions")
    entry_time = models.DateTimeField(auto_now_add=True)
    items_brought_in = models.TextField(help_text="List of items brought into the e-library")
    exit_time = models.DateTimeField(null=True, blank=True)
    items_taken_out = models.TextField(null=True, blank=True, help_text="List of items taken out when leaving")

    # Staff who processed entry/exit
    checked_in_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="checked_in_sessions"
    )
    checked_out_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, related_name="checked_out_sessions"
    )
    ...
    def __str__(self):
        return f"{self.visitor.full_name} - {self.entry_time.strftime('%Y-%m-%d %H:%M')}"
    ...
    @property
    def is_active(self):
        return self.status == "active" and self.exit_time is None

    def complete_session(self, items_taken_out="", checked_out_by=None):
        """Mark session as completed"""
        from django.utils import timezone
        self.exit_time = timezone.now()
        self.items_taken_out = items_taken_out
