...
class VisitorAdmin(SearchableModelAdmin):
    list_display = ("full_name", "department", "level", "registration_number", "active_phone_number", "created_at")
    search_fields = ("full_name", "department", "registration_number", "active_phone_number")
    ...
    fieldsets = (
        ("Personal Information", {"fields": ("full_name", "department", "level", "registration_number", "active_phone_number")}),
        ("Timestamps", {"fields": ("created_at", "updated_at"), "classes": ("collapse",)}),
    )


# ============================================================================
# E-LIBRARY SESSION ADMIN
# ============================================================================


@admin.register(ELibrarySession)
class ELibrarySessionAdmin(RobustModelAdmin):
    list_display = ("visitor_link", "entry_time_display", "status_colored", "duration_display", "checked_in_by", "checked_out_by")
    search_fields = ("visitor__full_name", "visitor__registration_number", "items_brought_in", "items_taken_out")
    list_filter = ("status", "entry_time", "exit_time", "checked_in_by", "checked_out_by")
    readonly_fields = ("entry_time", "exit_time", "duration", "session_items_display")
    date_hierarchy = "entry_time"
    list_per_page = 50

    fieldsets = (
        ("Session Information", {"fields": ("visitor", "entry_time", "exit_time", "status", "duration")}),
        ("Items Tracking", {"fields": ("session_items_display",), "classes": ("collapse",)}),
        ("Staff Processing", {"fields": ("checked_in_by", "checked_out_by")}),
        ("Notes", {"fields": ("notes",), "classes": ("collapse",)}),
    )

    def visitor_link(self, obj):
        return format_html(
            '<a href="/admin/catalog/visitor/{}/change/">{}</a>',
            obj.visitor.id,
    ...
    def entry_time_display(self, obj):
        return obj.entry_time.strftime("%b %d, %Y %H:%M")
    ...
    def session_items_display(self, obj):
        html = f"<strong>Brought In:</strong><br/>{obj.items_brought_in or 'None'}"
        if obj.items_taken_out:
            html += f"<br/><br/><strong>Taken Out:</strong><br/>{obj.items_taken_out}"
        return format_html(html)

    session_items_display.short_description = "Items"
